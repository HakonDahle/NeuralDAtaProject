...
def electrode_list(activeElectrodes,ti):    # Returns a list of timestamps organized for each electrode
    temp_array = []
    temp_electrode = []
    for electrodenr in range(0,64):
        for element in range(len(activeElectrodes)):
            ...
            if electrodenr == spike:
                temp_electrode.insert(len(temp_electrode),float(ti[element]))   # Inserts the timestamp into the column for the electrode
